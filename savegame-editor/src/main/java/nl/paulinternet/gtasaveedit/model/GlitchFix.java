...
public class GlitchFix {
    ...
    private static byte[] readResourceAsBytes(int length, String name) {
        byte[] res = new byte[length];
        try (DataInputStream in = new DataInputStream(GlitchFix.class.getResourceAsStream(name))) {
            in.readFully(res);
        } catch (Exception e) {
    ...
    public static void fixZoneGlitch() {
        // Load data
        if (zoneinfo1 == null || zoneinfo2 == null) {
            zoneinfo1 = readResourceAsBytes(12134, "/zoneinfo1.dat");
