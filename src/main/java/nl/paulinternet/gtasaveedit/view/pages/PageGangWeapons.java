...
public class PageGangWeapons extends Page
{
	...
	public PageGangWeapons () {
		...
		String[] gangs = new String[] {"Ballas", "Grove Street", "Los Santos Vagos", "San Fierro Rifa",
                "Da Nang Boys", "Italian Mafia", "Triads", "Varrio Los Aztecas"};
		...
		for (int i=0; i<gangs.length; i++) {
			...
			table.add(new WeaponBox(Model.vars.gangWeapon.get(i*3)), 1, i+1);
			table.add(new WeaponBox(Model.vars.gangWeapon.get(i*3+1)), 2, i+1);
			table.add(new WeaponBox(Model.vars.gangWeapon.get(i*3+2)), 3, i+1);
		}
