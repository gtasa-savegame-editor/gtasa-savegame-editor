...
public class Block21 implements Link
{
	...
	public void load (SavegameData io) {
		// Guess IPL version
		int version = Model.vars.version.getIntValue();
		int iplVersion = version == 0xfd148df6 || version == 0x5d31cc22 ? 2 : 1;
		
		Model.vars.currentIplVersion.setIntValue(iplVersion);
		Model.vars.convertIplVersion.setIntValue(iplVersion);
	}

	@Override
	public void save (SavegameData io) {
		int currentIpl = Model.vars.currentIplVersion.getIntValue();
		int convertIpl = Model.vars.convertIplVersion.getIntValue();
		...
		if (currentIpl != convertIpl) {
			...
			Model.vars.currentIplVersion.setIntValue(convertIpl);
		}
	}
}
